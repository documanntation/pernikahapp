...
export default function Header({ currentTab, setCurrentTab, showAdmin, setShowAdmin }: HeaderProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setCurrentTab('home')}>
            ...
            <div>
              <div className="flex items-baseline space-x-1">
                <span className="script-accent text-2xl font-bold tracking-wide text-brand-red">{t('appName')}</span>
                <span className="script-accent text-2xl text-brand-blue leading-none">{t('appSubtitle')}</span>
              </div>
          ...
          <nav className="hidden md:flex space-x-1 lg:space-x-2">
            <button
              id="nav-home"
              onClick={() => { setCurrentTab('home'); setShowAdmin(false); }}
            ...
            <button
              id="nav-about"
              onClick={() => { setCurrentTab('about'); setShowAdmin(false); }}
              className={`px-4 py-2 rounded-full text-xs font-semibold tracking-wider uppercase transition-all duration-300 ${
            ...
            <button
              id="nav-req-rec"
              onClick={() => { setCurrentTab('reqrec'); setShowAdmin(false); }}
              className={`px-4 py-2 rounded-full text-xs font-semibold tracking-wider uppercase transition-all duration-300 ${
            ...
            <button
              id="nav-coming-soon"
              onClick={() => { setCurrentTab('frameworks'); setShowAdmin(false); }}
              className={`px-4 py-2 rounded-full text-xs font-semibold tracking-wider uppercase transition-all duration-300 ${
            ...
            >
              {t('navInteractiveHub')} <span className="ml-1 text-[9px] px-1.5 py-0.5 rounded-full bg-brand-blue text-white scale-90">{t('navSoonBadge')}</span>
            </button>
          </nav>
          ...
          <div className="flex items-center space-x-2 lg:space-x-3">
            ...
            {/* Micro connectivity pills */}
            <div className="hidden sm:flex items-center space-x-1 bg-brand-beige-light/20 border border-brand-beige-dark/30 px-2.5 py-1.5 rounded-full text-[10px] font-sans">
              ...
              <span className="text-[#4b5563] font-semibold">
                {isFirebaseReady ? t('liveFirestore') : t('localSandbox')}
              </span>
            </div>
          </div>
        ...
        <div className="md:hidden flex justify-center py-2 border-t border-brand-beige-dark/20 space-x-1 overflow-x-auto pb-3">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold uppercase shrink-0 ${
              currentTab === 'home' && !showAdmin ? 'bg-brand-red text-white' : 'text-brand-slate'
            }`}
          >
            {t('navHome')}
          </button>
          <button
            ...
            onClick={() => { setCurrentTab('about'); setShowAdmin(false); }}
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold uppercase shrink-0 ${
              currentTab === 'about' && !showAdmin ? 'bg-brand-red text-white' : 'text-brand-slate'
            }`}
          >
            {t('navAboutUs')}
          </button>
          <button
            ...
            onClick={() => { setCurrentTab('reqrec'); setShowAdmin(false); }}
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold uppercase shrink-0 ${
              currentTab === 'reqrec' && !showAdmin ? 'bg-brand-red text-white' : 'text-brand-slate'
            }`}
          >
            {t('navReqRec')}
          </button>
          <button
            ...
            onClick={() => { setCurrentTab('frameworks'); setShowAdmin(false); }}
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold uppercase shrink-0 ${
              currentTab === 'frameworks' && !showAdmin ? 'bg-brand-red text-white' : 'text-brand-slate'
            }`}
          >
            {t('navInteractiveHub')}
          </button>
